...
export const testProxyUrl = async (proxyUrl, opt = {}) => {
...
export const saveDenyList = (filename) => {
    fs.writeFileSync(filename, JSON.stringify(proxyDenyList, null, 2));
};
export const loadDenyList = (filename) => {
    try {
...
export const saveAllowList = (filename) => {
    fs.writeFileSync(filename, JSON.stringify(proxyAllowList, null, 2));
};
export const loadAllowList = (filename) => {
    try {
        const data = fs.readFileSync(filename, 'utf8');
        const arr = JSON.parse(data);
        proxyAllowList = [...arr];
...
export const testProxyCache = async (opt = {}) => {
    if (opt.verboseLogging)
        console.warn('* testProxyCache', opt);
    if (!proxyCache.length) {
        await updateProxyCache();
    }
    if (opt.verboseLogging)
        console.warn(`* proxyCache.length: ${proxyCache.length}`);
    let testProxy = null;
    let index = 0;
    while (index <= proxyCache.length) {
        ...
        if (!opt.retestAllows && proxyAllowList.includes(testProxy.url)) {
            if (opt.verboseLogging)
                console.warn(`* skipping ${testProxy.url}, already allowlisted`);
            index++;
            continue;
        }
        if (!opt.retestDenies && proxyDenyList.includes(testProxy.url)) {
            if (opt.verboseLogging)
                console.warn(`* skipping ${testProxy.url}, already denylisted`);
            index++;
            continue;
        }
        if (opt.protocol && testProxy.protocol !== opt.protocol) {
            if (opt.verboseLogging)
                console.warn(`* skipping ${testProxy.url}, protocol mismatch`);
            index++;
            continue;
        }
        if (opt.verboseLogging)
            console.warn('* testing proxy', testProxy.url);
